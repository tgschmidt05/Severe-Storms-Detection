//tealium universal tag - utag.12 ut4.0.201602291956, Copyright 2016 Tealium.com Inc. All Rights Reserved.
var s_account="twciwund";var s=s_gi(s_account);s.dynamicAccountSelection=false;s.dynamicAccountList="";s.trackDownloadLinks=true;s.trackExternalLinks=true;s.trackInlineStats=true;s.linkDownloadFileTypes="csv,exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx";s.linkInternalFilters="javascript:,.wunderground.com,.wund.com,wxug.com,.fullscreenweather.com";s.linkLeaveQueryString=false;s.linkTrackVars="None";s.linkTrackEvents="None";s.usePlugins=false;s.currencyCode="USD";s.visitorNamespace="twci";s.trackingServer="odc.wunderground.com";s.trackingServerSecure="sodc.wunderground.com";s.dc=122;var s_code='',s_objectID;function s_gi(un,pg,ss){var c="s.version='H.25.4';s.an=s_an;s.logDebug=function(m){var s=this,tcf=new Function('var e;try{console.log(\"'+s.rep(s.rep(s.rep(m,\"\\\\\",\"\\\\"
...
try{(function(id,loader,u){try{u=utag.o[loader].sender[id]={}}catch(e){u=utag.sender[id]};u.ev={'view':1,'link':1,'video':1};u.o=s;u.pushlt=function(l,v){if(typeof l!="undefined")l.push(v)};u.map={"page_name":"pageName","channel":"channel","content_type":"prop1","sub_section":"prop2","member_type":"eVar12","search_term":"eVar11","search_type":"eVar17","search_error":"eVar18","s_socialShare":"eVar9","social_platform":"prop17","social_content_type":"prop16","event_type:Social Share":"event1","event_type:Share Button Click":"event23","event_type:WM Time Control Clicks":"event25","event_type:Document Level Click":"event20","wundermap_tc_feature":"eVar16","click_tracking_element":"eVar14","ad_blocker":"eVar10"};u.extend=[function(a,b){s.apl=new Function("l","v","d","u",""
...
+"ngth:x).toLowerCase()?v:0");s.c_r("sc_debug_tracking")&&(s.debugTracking=!0);s.manageVars=new Function("c","l","f",""
...
+"Of('D=')!=0){s[t]=s[t].toLowerCase();}}");s.addEvent=new Function("v",""
+"var s=this,i,n,a,m=0,L=s.events,d=',';if(L){a=L.split(d);for(i=0;i<"
+"a.length;i++){n=a[i];m=m||n==v;}}if(!m)L=L?L+d+v:v;s.events=L;retur"
+"n s.events");s.urlHelper=new Function(""
+"var s=this;s.url=s.pageURL?s.pageURL:s.wd.location.href;s.url=s.url"
+".toLowerCase();s.hash=s.wd.location.hash;s.uf=s.url.split('/');s.ul"
+"=s.uf.length;s.sda=s.uf[2].split('.');if(s.sda.length>2){s.sd=s.sda"
+"[0]}s.url=s.url.substring(0,(s.url.indexOf('#')==-1)?s.url.length:s"
+".url.indexOf('#'));s.url=s.url.substring(0,(s.url.indexOf('?')==-1)"
+"?s.url.length:s.url.indexOf('?'));s.fn=s.url.substring(s.url.lastIn"
+"dexOf('/')+1,s.url.length);s.cfn=s.fn.split('.')[0].replace(/_/gi,'"
+"-');if(s.uf[2]){s.u0=s.uf[2]}if(s.ul>4&&s.uf[3]){s.u1=s.uf[3]}if(s."
+"ul>5&&s.uf[4]){s.u2=s.uf[4]}if(s.ul>6&&s.uf[5]){s.u3=s.uf[5]}if(s.u"
+"l>7&&s.uf[6]){s.u4=s.uf[6]}if(s.ul>8&&s.uf[7]){s.u5=s.uf[7]}s.date="
+"new Date().valueOf()");s.screenOrientation=new Function(""
+"var a=window.matchMedia('(orientation: portrait)');if(a.matches){re"
+"turn'portrait'}else{return'landscape'}");s.touchScreen=new Function(""
+"var s=this,a='touch screen',b='non-'+a;if('createTouch' in s.d){ret"
+"urn a}else{return b}");s.handlePPVevents=new Function(""
...
+")?vh:cy)):'';s.c_w('s_ppv',cn);");s.getPercentPageViewed=new Function("pid",""
...
+")?(a):(a[1]);");s.getPreviousValue=new Function("v","c","e","m",""
...
+"n r}");s.getQueryParam=new Function("p","d","u","h","c",""
+"var s=this,v='',i,j,t;d=d?d:'';u=u?u:(s.pageURL?s.pageURL:s.wd.loca"
+"tion);if(u=='f')u=s.gtfs().location;while(p){i=p.indexOf(',');i=i<0"
+"?p.length:i;t=s.p_gpv(p.substring(0,i),u+'',h);if(t){t=t.indexOf('#"
+"')>-1?t.substring(0,t.indexOf('#')):t;}if(t)v+=v?d+t:t;p=p.substrin"
+"g(i==p.length?i:i+1)}if(!c){v=v.toLowerCase()}return v");s.p_gpv=new Function("k","u","h",""
...
+"var s=this,a=new Date,v=v?v:'',c=c?c:'s_gvo',e=e?e:0,i=t=='m'?6E4:8"
+"64E5;k=s.c_r(c);if(v){a.setTime(a.getTime()+e*i);s.c_w(c,v,e==0?0:a"
+");}return v==k?'':v");s.getTimeParting=new Function("f","z","l",""
+"var s=this,d,A,B,C,D,E,F,G,H,U,W,X,Y,Z;d=new Date();A=d.getFullYear"
+"();if(A=='2013'){B='10';C='03'}if(A=='2014'){B='09';C='02'}if(A=='2"
+"015'){B='08';C='01'}if(A=='2016'){B='13';C='06'}if(A=='2017'){B='12"
+"';C='05'}if(A=='2018'){B='11';C='04'}if(!B||!C){B='11';C='04'}B='03"
+"/'+B+'/'+A;C='11/'+C+'/'+A;D=new Date('1/1/2000');if(D.getDay()!=6|"
+"|D.getMonth()!=0){return 'data not available'}else{z=z?z:'-8';z=par"
+"seFloat(z);B=new Date(B);C=new Date(C);W=new Date();if(W>B&&W<C&&!l"
+"){z=z+1}W=W.getTime()+(W.getTimezoneOffset()*6E4);W=new Date(W+(36E"
+"5*z));X=['sunday','monday','tuesday','wednesday','thursday','friday"
+"','saturday'];E=B=W.getHours();C=W.getMinutes();if(C<10){C='0'+C};D"
+"=W.getDay();Z=X[D];A='weekday';if(D==6||D==0){A='weekend'}X='00';if"
+"(C>29){X='30'}U='am';if(B>=12){U='pm';B=B-12};if(B==0){B=12};F=W.ge"
+"tDate();G=W.getMonth()+1;H=W.getFullYear();W=B+':'+X+U;Y=C+U;if(f){"
+"if(f=='h'){return W}if(f=='m'){return B+':'+Y}if(f=='k'){return E+'"
+":'+X}if(f=='r'){return E+':'+C}if(f=='d'){return Z}if(f=='w'){retur"
+"n A}if(f=='f'){return Z+'|'+B+':'+Y}if(f=='j'){return G+'/'+F+'/'+H"
+";}}else{return Z+'|'+W}}");s.getVisitNum=new Function(""
+"var s=this,a=31536E6,b=18E5,d=new Date(),t=d.getTime(),v,V,c='s_vnu"
+"m',C='s_invisit';d.setTime(t+b);v=parseInt(s.c_r(c));V=s.c_r(C);if("
+"!v){s.c_w(C,1,d);d.setTime(t+a);s.c_w(c,1,d);return 1}else{if(!V){s"
+".c_w(C,1,d);d.setTime(t+a);s.c_w(c,v+1,d);return v+1}else{s.c_w(C,1"
+",d);return v}}");s.getActionDepth=new Function("c",""
+"var s=this,v=1,t=new Date;t.setTime(t.getTime()+18E5);c=c?c:'s_gad'"
+";if(!s.c_r(c)){v=1}if(s.c_r(c)){v=s.c_r(c);v++}if(!s.c_w(c,v,t)){s."
+"c_w(c,v,0)}return s.pageNum=v");s.getActionDepth();s.getDaysSinceLastVisit=new Function("c","x","r","f","g",""
+"var s=this,a=new Date(),b,d,e,h=18E5,j=2592E6,k=6048E5,l=864E5,m,t="
+"a.getTime(),v=['cookies not supported','first visit','more than 30 "
+"days','more than 7 days','less than 7 days','less than 1 day','zero"
+"'];c=c?c:'s_dslv';x=x?x:1095;v[1]=f?f:v[1];e=s.c_r(c);a.setTime((l*"
+"x)+t);if(e&&g){s.c_w(c,e,a)}else{s.c_w(c,t,a)}if(!e){m=1;b=v[1]}els"
+"e{d=t-e;if(d>h){b=Math.floor(d/l);if(d>j){m=2}else if(d<j+1&&d>k){m"
+"=3}else if(d<k+1&&d>l){m=4}else if(d<l+1){m=5;b=v[6]}}}if(s.c_r(c))"
+"{if(r){return b}else if(m){return v[m]}else{return ''}}else{return "
+"v[0]}");s.manageVars=new Function("c","l","f",""
+"var s=this,vl,la,vla;l=l?l:'';f=f?f:1 ;if(!s[c])return false;vl='pa"
+"geName,purchaseID,channel,server,pageType,campaign,state,zip,events"
+",products,transactionID';for(var n=1;n<76;n++){vl+=',prop'+n+',eVar"
+"'+n+',hier'+n;}if(l&&(f==1||f==2)){if(f==1){vl=l;}if(f==2){la=s.spl"
...
+");return true;}else{return false;}");s.clearVars=new Function("t","var s=this;s[t]='';");s.lowercaseVars=new Function("t",""
+"var s=this;if(s[t]&&t!='events'){s[t]=s[t].toString();if(s[t].index"
+"Of('D=')!=0){s[t]=s[t].toLowerCase();}}");function s_getObID(o){var ID=o.href;return ID}s.getObjectID=s_getObID;s.setupDynamicObjectIDs=new Function(""
...
+"]=new Function('e',x)}}}s.wd.s_semaphore=0;return true");s.exitLinkHandler=new Function("p",""
...
+"ot(o);n=s.oid(o);x=o.s_oidt}}return s.oh(o);");s.m_Media_c="var m=s.m_i('Media');if(m.completeByCloseOffset==undefined)m.completeByCloseOffset=1;if(m.completeCloseOffsetThreshold==undefined)m.completeCloseOffsetThreshold=1;m.cn=function(n){var m="
...
+"addEventListener)s.wd.addEventListener('load',m.as,false);if(m.onLoad)m.onLoad(s,m)";s.m_i("Media");},function(a,b){s.usePlugins=true;function s_doPlugins(s)
{s.urlHelper();s.setupDynamicObjectIDs();if(!s.pageName)
{s.pageName=s.url}
s.server=s.u0;if(s.prop1=="error")
{s.pageType="errorPage";s.pageName="";s.prop7="page: error|"+s.url}
s.prop5=s.getPreviousValue(s.pageName,'','','no previous page name');s.prop6=s.getPercentPageViewed();if(!s.prop7)
{s.prop7="page: "+s.pageName}
s.prop8=s.touchScreen();var campaignCode=s.getQueryParam("cm_ven");if(campaignCode){s.eVar23=s.pageName+"?cm_ven="+campaignCode;}
s.eVar24=s.url;if(!s.campaign)
{s.campaign=s.getQueryParam('cm_ven,cm_cat,cm_pla,cm_ite',':')}
s.campaign=s.getValOnce(s.campaign,'s_campaign');s.eVar1=s.pageName?"..pageName":"";s.eVar2=s.prop1?"..c1":"";s.eVar3=s.channel?"..ch":"";s.eVar4=s.prop2?"..c2":"";s.eVar5=s.getTimeParting();if(s.pageNum==1)
{s.eVar6=s.getVisitNum();s.eVar7="1"==s.eVar6?"new":"repeat";s.eVar8=s.getDaysSinceLastVisit();s.addEvent('event17')}
if(s.pageNum==2)
{s.addEvent('event18')}
s.searchTerm=s.getValOnce(s.eVar11);s.searchType=s.eVar17;s.eVar17="";if(s.searchTerm)
{s.addEvent('event5');s.eVar17=s.searchType}
if(s.eVar18)
{s.addEvent('event22')}
s.eVar20=s.prop5?"..c5":"";s.eVar21="+1";s.eVar22="+1";s.addEvent('event15');if(s.getQueryParam('mr'))
{s.addEvent('event16')}
var s_elink=s.exitLinkHandler();if(s_elink)
{var appStore=false;s.prop7="action: exit|"+s_elink;if(s_elink.match(/market\.android\.com|play\.google\.com/))
{appStore="google play"}
else if(s_elink.match(/(phobos|itunes)\.apple\.com/))
{appStore="apple itunes"}
if(appStore)
{s.linkTrackVars+="events";s.linkTrackEvents="event19";s.prop7="action: "+appStore+" app store exit";s.eVar13=appStore+" app store";s.addEvent('event19')}}
s.manageVars('lowercaseVars');s.plugins=""}
s.doPlugins=s_doPlugins;},function(a,b){s.loadModule('Media');s.Media.autoTrack=false;s.Media.trackWhilePlaying=true;s.Media.trackVars="events,prop1,prop3,prop4,prop5,prop6,prop7,eVar2,eVar15,eVar16,prop51";s.Media.trackEvents="event8,event9,event10,event11,event12,event13,event14";s.Media.segmentByMilestones=true;s.Media.trackUsingContextData=true;s.Media.trackMilestones="25,50,75";s.Media.contextDataMapping={"a.media.name":"prop4,eVar15","a.media.segment":"eVar16","a.contentType":"prop1,eVar2","a.media.timePlayed":"event8","a.media.view":"event9","a.media.complete":"event10","a.media.segmentView":"event11","a.media.milestones":{25:"event12",50:"event13",75:"event14"}}
s.Media.monitor=function(s,media)
{var ma="action: video ";if((media.event=="MILESTONE")&&(media.eventFirstTime))
{if(media.milestone==25)
{s.prop7=ma+"25% viewed|"+media.name;fireRequest=true}
if(media.milestone==50)
{s.prop7=ma+"50% viewed|"+media.name;fireRequest=true}
if(media.milestone==75)
{s.prop7=ma+"75% viewed|"+media.name;fireRequest=true}
if(fireRequest)
{fireRequest=false;sendRequest()}}
if(media.event=="OPEN")
{s.prop7=ma+"started|"+media.name;sendRequest();s.prop7=""}
if(media.event=="CLOSE")
{s.prop7=ma+"completed|"+media.name;sendRequest();s.prop7=""}
function sendRequest()
{s.Media.track(media.name)}}}];u.send=function(a,b,c,d,e,f,g,h,ev,evt){if(u.ev[a]||typeof u.ev.all!="undefined"){utag.DB("send:12");b.sc_events=b.sc_events||{};u.addEvent=function(v){var t=[];if(v instanceof Array){t=v.slice(0);}else{t.push(v);}
...
for(d in c){if(typeof c[d]!='function'){if(typeof t[d]!='undefined'&&t[d]!=null&&t[d]!=''&&t[d].toString().indexOf('D=')!=0)
...
utag.DB("send:12:COMPLETE");}}
try{utag.o[loader].loader.LOAD(id)}catch(e){utag.loader.LOAD(id)}})('12','weather.wunderground');}catch(e){};
...
function mOver()
{
	...
	overFl = true;
	TweenLite.to(img_sky, 0.4, {scale: 1, y:0, ease:Power2.easeOut});
	TweenLite.to(img_water, 0.4, {y: img_water.sy-0, ease:Power2.easeOut});
	TweenLite.to(img_ship, 0.4, {y: img_ship.sy+5-0,  scale:0.6, ease:Power2.easeOut});
	...
	if(shoot==false)
	{
		...
		TweenLite.to(tTextPlate, 0.2, {y:-26, ease:Sine.easeOut,delay:0.2});
...
function mOut()
{
	...
	if(shoot==false)
	{
		...
		TweenLite.to(img_ship, 0.4, {y: img_ship.sy, scale:1/1.5, ease:Power2.easeOut});
		...
		TweenLite.to(textPlate, 0.2, {y:h-30, ease:Sine.easeOut});
		TweenLite.killTweensOf(tTextPlate);	
		TweenLite.killTweensOf(img_logo);	
...
function setStartPos()
{
	...
	img_torpIcon.x=w-46-6;
	img_torpIcon.sy=h-46-6;
...
function toPackshot()
{
	...
	TweenLite.to(tTextPlate, 0.2, {y:-60, ease:Sine.easeOut});
	TweenLite.to(img_logo, 0.5, {delay:1.1, y: 90, alpha:1, ease:Power2.easeOut, onUpdate:drawLogo, onUpdateParams:[ctxLogo,img_logo]});
	...
	TweenLite.to(Pattern.tPattern, 1.1, {alpha: 1, delay:1, ease:Power2.easeOut});
		...
		{	
			...
			if(torp3D.y<img_ship.y&&torp3D.x+torpedArray[t].dx>(img_ship.x+w/2+4)&&torp3D.x+torpedArray[t].dx<(img_ship.x+w/2+4)+448*img_ship.scale&&torpedArray[t].enable==true)
			{
				...
				if(img_ship.hp==0&&img_ship.exp==false&&autoPlay==false)
				{
					...
					var obj = {};
					obj.x = img_ship.x+w/2+160-20;
					obj.y = img_ship.y-30;
					obj.scale = 0.9;
					addExpArray.push(obj);
					obj = {};
					obj.x = img_ship.x+w/2+130-20;
					obj.y = img_ship.y-30;
					obj.scale = 1.1;
					addExpArray.push(obj);
					obj = {};
					obj.x = img_ship.x+w/2+180-20;
					obj.y = img_ship.y-20;
					obj.scale = 0.7;
					addExpArray.push(obj);
					obj = {};
					obj.x = img_ship.x+w/2+90-20;
					obj.y = img_ship.y-30;
					obj.scale = 0.7;
					addExpArray.push(obj);
					topTextState = 2;
